 ...
 *
 * <h2>Why this is not `page.tsx` any more</h2>
 *
 * <p>Because `useClerk()` throws when there is no `<ClerkProvider>` above it,
 * and during a production prerender there is not one — so `next build` failed
 * on this route with "useClerk can only be used within the <ClerkProvider />
 * component."
 *
 * <p>The provider is mounted by `AuthProvider`, which branches on `AUTH_MODE`:
 * in clerk mode through a `next/dynamic` with `ssr: false`, which renders
 * nothing on the server; in dev mode not at all, because dev mode has no Clerk.
 * Either way nothing server-side provides the context this file's first line
 * demands. A clerk-mode build only appeared to work because that `ssr: false`
 * boundary happened to swallow the page as well; a dev-mode build rendered the
 * page for real and the latent fault became the blocker.
 *
 * <p>So the route is now a shell — `page.tsx`, which calls no Clerk hook — and
 * this is loaded by it with `ssr: false` after hydration, when the provider
 * exists. Nothing in here changed: every branch, the watchdog, the navigation
 * mapping and the captcha element are the file that was here, moved.
 */
